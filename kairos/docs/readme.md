...
The following models are the closest publicly documented precedents for training in the sub-50M dense / low-active-param regime.

| Model | Params (total) | Params (active) | Tokens seen | Dataset | Batch size | Steps | Architecture | Key takeaway for Kairos |
|---|---|---|---|---|---|---|---|---|
| **Pythia-14M** | 14M | 14M (dense) | ~300B | The Pile (825 GB, 22 sources) | 2M tokens | 143 000 | GPT-NeoX, 6L × d128 × 4h, Flash Attention | 300B tokens on 14M params = $D/N \approx 21{,}000\times$ — proves aggressive over-training is standard at this scale. |
| **Pythia-31M** | 31M | 31M (dense) | ~300B | The Pile | 2M tokens | 143 000 | GPT-NeoX, 6L × d256 × 8h | Non-embedding params only 4.7M — direct comp to $N_\text{act}=4\text{M}$. |
| **TinyStories-1M** | ~1M | ~1M (dense) | ~3B (synthetic) | TinyStories | — | — | Transformer, ~8L × d64 | Sets a lower bound: coherent narrow-domain text is possible well below 4M params with curated data. |
| **TinyStories-33M** | ~33M | ~33M (dense) | ~3B (synthetic) | TinyStories | — | — | Transformer, larger variant | Domain-scoped training with little data leaves the model brittle outside its domain. |

**Reading these for Kairos:** Pythia-14/31M are the best compute analogues; TinyStories shows the floor. None of these is MoE — the $N_\text{total}=200\text{M}$ backward bottleneck has no precedent here.
