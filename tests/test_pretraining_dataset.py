...
def all_kinds_examples(rng):
    return [
        ...
        {"kind": "audio_caption", "audio": rng.uniform(-1, 1, 4000).astype(np.float32), "sample_rate": 8000, "caption": "a bark"},
...
def test_all_kinds_cover_expected_modalities(tokenizer, all_kinds_examples):
    ...
    assert seen == {
        int(Modality.TEXT), int(Modality.IMAGE), int(Modality.AUDIO),
        int(Modality.VIDEO), int(Modality.LIDAR), int(Modality.STATE), int(Modality.ACTION),
    }
...
def test_multimodal_padding_uses_text_modality(tokenizer, all_kinds_examples):
    ds = KairosPretrainingDataset(multimodal_examples=all_kinds_examples[:1], tokenizer=tokenizer, max_len=4096, stride=1)
    item = ds[0]
